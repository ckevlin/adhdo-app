...
export default function ADHDo() {
  ...
  const [loading, setLoading] = useState(true); // Start as true to prevent empty state flash
...
{"headline":"funny celebratory headline, 5-8 words max, no emoji","subtitle":"short supportive message","gifSearch":"2-3 word gif search term like 'mic drop' or 'happy dance' or 'couch relaxing'"}`
  ...
  const getSuggestion = async (skipTodayDoneCheck = false) => {
    ...
    try {
      ...
      let energyLevel, energyGuidance;
      if (currentHour >= 5 && currentHour < 9) {
        energyLevel = 'early morning (waking up)';
        energyGuidance = 'Energy is LOW and building. Suggest small, easy, low-friction tasks to build momentum. Quick wins only! Avoid anything that requires heavy thinking or lots of steps.';
      } else if (currentHour >= 9 && currentHour < 12) {
        energyLevel = 'mid-morning (energy building)';
        energyGuidance = 'Energy is RISING. Good time for tasks that need some focus. Can handle moderate complexity but still favor clear, actionable tasks.';
      } else if (currentHour >= 12 && currentHour < 15) {
        energyLevel = 'early afternoon (peak energy)';
        energyGuidance = 'Energy is at PEAK. This is the time for harder tasks, phone calls, complex work, or things they have been avoiding. Tackle the big stuff now!';
      } else if (currentHour >= 15 && currentHour < 18) {
        energyLevel = 'late afternoon (energy fading)';
        energyGuidance = 'Energy is DECLINING. Favor medium-effort tasks. Avoid starting anything too complex. Good for wrapping things up or routine tasks.';
      } else if (currentHour >= 18 && currentHour < 21) {
        energyLevel = 'evening (low energy, winding down)';
        energyGuidance = 'Energy is LOW. Only suggest easy, calming tasks. Home tasks, simple tidying, or relaxing activities. Nothing stressful or demanding!';
      } else {
        energyLevel = 'night (minimal energy)';
        energyGuidance = 'Energy is MINIMAL. Only suggest the easiest possible tasks if any. Better to rest. If they must do something, make it tiny and effortless.';
      }

      const systemPrompt = `You are an ADHD-friendly task assistant. Your goal is to suggest ONE task at a time, unless tasks are TRULY related.

ENERGY-BASED SELECTION (IMPORTANT):
Current energy: ${energyLevel}
${energyGuidance}

BATCHING RULES - BE STRICT:
Only batch tasks if they are DIRECTLY related to the SAME person, place, or action:
- SAME PERSON: "Email John", "Call John" → YES, batch (both about John)
- SAME PLACE: "Get milk at grocery", "Get eggs at grocery" → YES, batch (same store)
- SAME ACTION TYPE: "Call bank", "Call doctor", "Call mom" → YES, batch (all phone calls)
- SAME ERRAND RUN: "CVS", "Target" (if nearby) → YES, batch

DO NOT batch just because tasks are vaguely similar:
- "Clean bathroom", "Do laundry" → NO, different tasks
- "Email boss", "Call mom" → NO, different people
- "Order shoes", "Research vacations" → NO, not related

When in doubt, suggest ONE task only. Less is more for ADHD brains.

SELECTION PRIORITY:
1. Match the user's current energy level (most important!)
2. Prefer urgent tasks when energy-appropriate
3. Only batch if tasks are truly related (same person/place/action)
4. Quick wins build momentum - especially important when energy is low

Be supportive, never judgmental. Your headline should match the energy vibe (calm and gentle when energy is low, more pumped up when energy is high).`;

      // Helper to estimate task size info
      const getTaskDetails = (t) => {
        let details = `ID: ${t.id} | "${t.text}" | urgent: ${t.urgent || false}`;
        if (t.subtasks && t.subtasks.length > 0) {
          const incomplete = t.subtasks.filter(st => !st.completed).length;
          details += ` | subtasks: ${incomplete} remaining`;
        }
        if (t.notes) {
          details += ` | notes: "${t.notes.substring(0, 100)}${t.notes.length > 100 ? '...' : ''}"`;
        }
        if (t.phone) details += ` | has phone number`;
        if (t.address) details += ` | has address`;
        return details;
      };
...
${priorityPool.map(t => `- ${getTaskDetails(t)}`).join('\n')}

ALL OTHER TASKS (can batch with priority tasks if related):
${incomplete.filter(t => !priorityPool.includes(t)).map(t => `- ${getTaskDetails(t)} | section: ${t.doDate ? (t.doDate <= todayDate ? 'today' : t.doDate === tomorrowDate ? 'tomorrow' : t.doDate <= weekEndDate ? 'week' : 'later') : 'void'}`).join('\n')}

TASK SIZE ESTIMATION:
- Look at the task title: simple verbs like "text", "email", "check" = small; "call", "schedule", "order" = medium; "research", "organize", "plan", "clean" = larger
- Subtasks count: 0-1 = small, 2-3 = medium, 4+ = large
- Notes length: short/none = small, detailed notes = likely more complex
- Has phone/address = likely involves coordination or travel

Match task size to current energy level!

Return ONLY JSON:
{
  "taskId": "primary task ID - pick one that matches current energy level",
  "headline": "motivating 5-10 words. NO HYPHENS OR DASHES. Match the energy vibe (gentle when tired, energetic at peak)",
  "subtitle": "supportive message acknowledging their energy level",
  "batchTaskIds": [] OR ["oneRelatedTaskId"] - ONLY include if truly related (same person/place/action). Usually empty!,
  "batchReason": "short label explaining relation like 'Both for John' or 'Phone calls'" or null
}`;
...
Return ONLY JSON:
{
  ...
  const completeTask = async (id) => {
    ...
    setTimeout(async () => {
      try {
        ...
        console.error('Firebase complete failed:', e);
  ...
  const uncompleteTask = async (id) => {
    try {
      ...
      console.error('Firebase uncomplete failed:', e);
  ...
  const toggleUrgent = async (id) => {
    ...
    try {
      await taskService.updateTask(id, { urgent: !task.urgent });
    } catch (e) {
      console.error('Firebase toggle urgent failed:', e);
  ...
  const toggleSubtask = async (taskId, subtaskIndex) => {
    ...
    try {
      ...
      console.error('Firebase toggle subtask failed:', e);
  ...
  const scheduleTask = (taskId, when) => {
    ...
    try {
      taskService.updateTask(taskId, { doDate });
    } catch (e) {
      console.error('Firebase schedule failed:', e);
    }
  ...
  const handleDrop = (e, sectionKey, dropIndex = null) => {
    ...
    if (isSameSection && dropIndex !== null) {
      ...
      // Also update in Firebase
      try {
        taskService.updateTask(draggedTask.id, { doDate: newDoDate });
      } catch (e) {
        console.error('Firebase drag update failed:', e);
      }
          ...
          <div style={{ textAlign: 'center' }}>
            {!settings.apiKey ? (
              ...
              <div style={{ 
                ...
                <h2 style={{ fontSize: 26, fontWeight: 700, margin: 0, lineHeight: 1.3 }}>
                  {isEvening ? "You're all set for tonight" : "Nothing on your plate"}
                </h2>
                {isEvening && (
